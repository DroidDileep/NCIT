<?php #archieve unwanted logs based on teacher name and date;delete approved and not approved logs seperatly
...
<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(!empty($_POST['tname'])){
			...
			$query="SELECT * FROM logsheet where tname='$tname' and status='not approved' and date between '$from' AND '$to' ORDER BY date DESC";

			#get logs that are approved
			$query2="SELECT * FROM logsheet where tname='$tname' and status='approved' and date between '$from' AND '$to' ORDER BY date DESC";
		}
		else{
			#logs of all teachers
			#get logs that are not approved
			$query="SELECT * FROM logsheet where status='not approved' and date between '$from' AND '$to' ORDER BY date DESC";

			#get logs that are approved
			$query2="SELECT * FROM logsheet where status='approved' and date between '$from' AND '$to' ORDER BY date DESC";
		}
